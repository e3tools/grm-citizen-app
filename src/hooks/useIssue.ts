...
export function useIssue(id: number) {
  ...
  const [loadingIssueCommentsMore, setLoadingIssueCommentsMore] =
    useState(false)
  ...
  const [loadingIssueAttachmentsMore, setLoadingIssueAttachmentsMore] =
    useState(false)
  const [currentIssueAttachmentsPage, setCurrentIssueAttachmentsPage] =
    useState(1)
  ...
  const loadMoreIssueComments = async (id: number) => {
    ...
    try {
      ...
      setIssueComments(prev => [
        ...prev,
        ...(issueComments.results || issueComments),
      ])
  ...
  const loadMoreIssueAttachments = async (id: number) => {
    ...
    try {
      ...
      setIssueAttachments(prev => [
        ...prev,
        ...(issueAttachments.results || issueAttachments),
      ])
