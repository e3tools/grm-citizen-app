...
const IssueAttachments = ({issueId, attachments, attachmentsHasNextPage, loadingIssueAttachmentsMore, loadMoreIssueAttachments}) => {
    const handleScroll = useCallback(({ nativeEvent }) => {
        ...
        if (isCloseToBottom && attachmentsHasNextPage && !loadingIssueAttachmentsMore) {
          loadMoreIssueAttachments(issueId);
        }
    }, [attachmentsHasNextPage, loadingIssueAttachmentsMore, loadMoreIssueAttachments, issueId]); // Added issueId to dependencies
                      ...
                      {attachments && attachments.length > 0 && attachments.map((attachment, index) => {
                          ...
                          return (
                              ...
                              key={index}>
                                  ...
                                  { attachment.file && !attachment.file.toLowerCase().includes('jpeg') ?
                                    (
